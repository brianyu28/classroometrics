...
class ElementService:
    ...
    def reset_elements_to_default(dashboard: Dashboard):
        ...
        for element in dashboard.elements.all():
            element.delete()
        default_elements = [
            Element(icon="sentiment.happy", identifier="sentiment.positive", name="", section=0, order=0, is_visible=True),
            Element(icon="sentiment.neutral", identifier="sentiment.neutral", name="", section=0, order=1, is_visible=True),
            Element(icon="sentiment.sad", identifier="sentiment.negative", name="", section=0, order=2, is_visible=True),

            Element(icon="completion.yes", identifier="completion.yes", name="", section=0, order=3, is_visible=False),
            Element(icon="completion.no", identifier="completion.no", name="", section=0, order=4, is_visible=False),
