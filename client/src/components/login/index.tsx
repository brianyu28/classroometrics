...
function Login({ onUpdateUserAuth: setUserAuth }: LoginProps) {
  const [error, setError] = useState(null);
  const [classroom, _setClassroom, updateClassroom] = useInputFieldState("");
    ...
    return (
      <>
        ...
        <form onSubmit={submitClassroomForm}>
          ...
          />
          <input disabled={!canSubmitStudentForm} type="submit" value="Join" />
        </form>
      </>
    );
  };
  ...
  return (
    <div className="login-container">
      ...
      <br />
      {renderJoinClassroomForm()}
      {renderLoginForm()}
      <br />
      <p>
        Questions? Contact{" "}
        <a href="mailto:brian@brianyu.me?subject=Classroometrics">Brian Yu</a>.
      </p>
    </div>
  );
}
