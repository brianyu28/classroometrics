...
function RoomManager() {
  ...
  const [isShowingActivityView, toggleIsShowingActivityView] =
  ...
  const removeExpiredElementActivity = () => {
    ...
    setElementActivity((elementActivity) =>
      elementActivity.filter(
        (activity) => timestamp - activity.timestamp < ACTIVITY_LIFESPAN_SECONDS
      )
    );
  };
  ...
  const updateVisibilityForElement = (
    elementId: number,
    isVisible: boolean
  ) => {
    const updatedRoom = {
  ...
  const deleteElement = (elementId: number) => {
    const updatedRoom = {
      ...room,
      groups: room.groups.map((group) =>
        group.filter((element) => element.id !== elementId)
      ),
    };
    setRoom(updatedRoom);
    saveUpdatedRoom(updatedRoom);
  ...
  const updateGroup = (groupIndex: number, updatedGroup: Element[]) => {
    const updatedRoom = {
      ...room,
      groups: room.groups.map((group, index) =>
      ...
      <div>
        <div>
          <label>
            <input
              ...
              onChange={toggleQuestionsEnabled}
            ></input>
            Responses enabled?
          </label>
        </div>
        <div>
          <label>
            <input
              ...
              onChange={toggleQuestionsVisible}
            ></input>
        ...
        <div>
          <label>
            <input
              type="checkbox"
              checked={isInBatchToggleMode}
              onChange={switchBatchToggle}
            ></input>
